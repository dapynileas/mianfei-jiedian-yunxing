...
> A compatibility-first repository for readers checking which client should open today’s article first.
...
| --- | --- |
| 更新日期 | 2026年9月14日 |
| 自动同步时间 | 2026年9月14日 00:00 |
| 当天文章 | https://free.tntyx.com/posts/nodes/2026-09-14 |
| 订阅格式 | XBoard 通用订阅 |
...
```text
https://free.tntyx.com/posts/nodes/2026-09-14
```

请打开上面的博客文章，在正文“免费订阅地址”区域获取当天可用入口。兼容性说明放在正文中更好维护，也更容易保持更新一致。

## 快速入口

- 今日免费节点文章：https://free.tntyx.com/posts/nodes/2026-09-14
...
- 面向场景：多客户端兼容性和导入路径选择入口
- 推荐读者：不确定应该用 Clash、Mihomo、v2rayN 还是 Shadowrocket 的读者
- 内容重点：客户端选择、订阅格式识别、导入步骤和常见失败原因；2026年9月14日更新；香港、日本、新加坡、美国 等地区；100 Mbps 公共限速
- 固定入口：https://free.tntyx.com/

## 今日导入路径

1. 打开今日文章，确认日期是 2026年9月14日，再获取正文里的免费订阅入口。
2. 先按客户端专题选择入口，再打开今日文章复制对应订阅地址；如果客户端没有弹出，就复制文章里的地址后手动新增订阅。
...
本站每日北京时间 00:00 自动处理免费节点用户：重置流量、刷新订阅 Token、写入服务端动态数据，并同步 GitHub 分享仓库。先按客户端选择入口，再去当天文章复制链接。 仓库里的 `sub/latest.txt` 只保存当天文章链接，不保存真实订阅地址。

免费节点适合这些场景：

- 临时测试 Clash、Mihomo、V2Ray、v2rayN 等客户端是否配置正确。
- 学习订阅导入、节点切换、规则模式和全局模式的基础流程。
- 短时间备用连接，或在不同设备上验证客户端兼容性。

不建议用于这些场景：

- 支付、银行、交易平台等敏感账号操作。
- 企业后台、长期办公账号和重要业务连接。
- 对稳定性、独享资源和售后响应有明确要求的主力线路。
